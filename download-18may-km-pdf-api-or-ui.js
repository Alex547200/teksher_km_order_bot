...
const LIST_ENDPOINT = "/facade/api/v1/operations/filter?size=15&page={page}&startDate=2026-05-18&endDate=2026-05-19";
const TARGET_DATE = "2026-05-18";
const TARGET_OPERATION_TYPE = "MARK_CODE_ORDER";
const API_OUTPUT_DIR = path.join(os.homedir(), "Desktop", "Текшер PDF", "18.05.2026_API");
const UI_OUTPUT_DIR = path.join(os.homedir(), "Desktop", "Текшер PDF", "18.05.2026");
const LOG_PATH = path.join(PROJECT_DIR, "download_18may_km_pdf_log.json");
  ...
  async function fetchPageWithRefresh(pageNum) {
    ...
    try {
      const refreshed = await authHelper.refreshAuthToken(refreshToken, {
        ...
        source: "download-18may-km-pdf-api-or-ui",
      });
...
async function dumpUiDebug(page, name, extraText = "") {
  const debugDir = path.join(PROJECT_DIR, "debug", "download-18may-km-pdf-ui");
